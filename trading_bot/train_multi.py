#!/usr/bin/env python
from env.trading_env_multi import TradingEnv
import configparser
...
model.learn(total_timesteps=int(config['MAIN']['TrainSteps']), tb_log_name=config['ENV']['ObsSpace']+config['ENV']['ActionSpace']+str(len(config['ENV']['Commodities'].split(',')))+'_'+str(str(args.seed)))

model_save = "./Models/"+config['MAIN']['Model']+"-" + config['ENV']['TradingStrategy'] + config['ENV']['ObsSpace'] + config['ENV']['ActionSpace']+str(args.seed) + ".h5"
